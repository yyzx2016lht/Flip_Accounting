...
class OverlayManager(private val ctx: Context) {
    ...
    private fun saveJsonToLocal(obj: JSONObject) {
        ...
        var categoryName = tao.test.tapaccounting.logic.CategoryNameNormalizer.normalizeForStorage(obj.optString("category_name", ""))
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            var toAssetObj: tao.test.tapaccounting.data.local.entity.Asset? = null
            if (typeIndex == 2 || typeIndex == 3) {
                val asset2 = obj.optString("to_asset_name", "")
            ...
            val exchangeRate = when {
                typeIndex == 2 && toAssetObj != null && amount > 0.0 ->
                    tao.test.tapaccounting.logic.BillAssetImpactService.estimateExchangeRateToTarget(
                        amount,
                        effectiveCurrency,
                        toAssetObj.currency
                    )
                effectiveCurrency == "CNY" -> 1.0
                else -> tao.test.tapaccounting.logic.BillAssetImpactService.estimateExchangeRateToCny(effectiveCurrency)
            }
            ...
            tao.test.tapaccounting.logic.BillMutationService.insertBillAndApplyImpact(db, bill)
        }
    }
