package tao.test.tapaccounting.ui.main.assets
...
internal class AssetBillDetailSheetController(
    ...
    fun showBillDetailSheet(bill: Bill, displayAssetId: Long = detailOwnerAssetId(bill)) {
        if (Prefs.isIndependentDetailEnabled(activity) && bill.id > 0L) {
            tao.test.tapaccounting.ui.activity.BillDetailActivity.start(activity, bill.id)
            return
        }
        ...
        if (isTransfer) {
            ...
            if (isRefund) {
                ...
                scope.launch(Dispatchers.IO) {
                    val original = tao.test.tapaccounting.logic.BillMutationService.resolveRefundSourceBill(db, bill)
                    withContext(Dispatchers.Main) {
                        if (original != null) {
