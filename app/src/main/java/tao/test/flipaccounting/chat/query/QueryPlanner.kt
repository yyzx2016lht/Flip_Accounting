...
import tao.test.flipaccounting.cleanJsonString
import tao.test.flipaccounting.extractFirstJsonObjectText
...
class QueryPlanner(
    ...
    suspend fun plan(userText: String, context: QueryContext): QueryAction {
        if (AiIntentRouter.isHighRiskWrite(userText) || looksLikeWrite(userText)) {
    ...
    private fun looksLikeWrite(text: String): Boolean {
        return listOf("修改", "改成", "改为", "删除", "清空", "覆盖", "新增", "记一笔", "记账")
