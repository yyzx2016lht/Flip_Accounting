...
class OverlayManager(private val ctx: Context) {
    ...
    fun showOverlay(prefill: JSONObject? = null, showSaveOnly: Boolean = false) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !android.provider.Settings.canDrawOverlays(ctx)) {
            android.widget.Toast.makeText(ctx, "请在设置中开启悬浮窗权限，以便使用 AI 辅助功能", android.widget.Toast.LENGTH_LONG).show()
    ...
    private fun saveJsonToLocal(obj: JSONObject) {
        ...
        var categoryName = obj.optString("category_name", "").replace("/::/", " > ").replace("/:::/", " > ")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            var toAssetObj: tao.test.flipaccounting.data.local.entity.Asset? = null
            if (typeIndex == 2 || typeIndex == 3) {
                val asset2 = obj.optString("to_asset_name", "")
            ...
            tao.test.flipaccounting.logic.BillMutationService.insertBillAndApplyImpact(db, bill)
        }
    }
