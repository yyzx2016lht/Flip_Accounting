...
class TapDetector(
    ...
    fun start(): Boolean {
        ...
        try {
            ...
            tap = when {
                lowPowerMode && tripleEnabled -> HeuristicTapTapTapRT(160000000L, true, TapRT.HEURISTIC_MIN_TIME_GAP_NS).apply {
                ...
                lowPowerMode && !tripleEnabled -> TapRT(160000000L, TapRT.HEURISTIC_MIN_TIME_GAP_NS).apply {
                    getLowpassKey().setPara(0.2f)
                    getHighpassKey().setPara(0.2f)
                    getPositivePeakDetector().setMinNoiseTolerate(sensitivity)
                    getPositivePeakDetector().setWindowSize(64)
