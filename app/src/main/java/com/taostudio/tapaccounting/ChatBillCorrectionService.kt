...
class ChatBillCorrectionService(
    ...
    suspend fun processBillModifyResult(result: JSONObject, userText: String, oldBill: Bill) {
        ...
        val categoryName = com.taostudio.tapaccounting.logic.CategoryNameNormalizer.normalizeForStorage(result.optString("category_name", oldBill.categoryName)).trim().ifBlank { oldBill.categoryName }
        ...
        val savedBill = withContext(Dispatchers.IO) {
            com.taostudio.tapaccounting.logic.BillMutationService.replaceBill(db, oldBill, newBill)
        }
