...
class CreditCardCycleService {
     ...
     */
    fun calculateSnapshot(asset: Asset, bills: List<Bill>): CreditCardCycleSnapshot? {
        ...
        // 还款（在已出账周期内）
        val paymentsInCycle = billedBills
        ...
        val availableLimit = if (asset.creditLimit > 0) {
            asset.creditLimit - amountDue - unbilledSpend
        } else null

        val daysToDue = if (dueDay > 0) {
            calculateDaysToDue(dueDay, now)
        } else null
     ...
     * 计算距离还款日还有几天。
     */
    private fun calculateDaysToDue(dueDay: Int, now: Long): Int {
        val cal = Calendar.getInstance()
        cal.timeInMillis = now
        val currentDay = cal.get(Calendar.DAY_OF_MONTH)
