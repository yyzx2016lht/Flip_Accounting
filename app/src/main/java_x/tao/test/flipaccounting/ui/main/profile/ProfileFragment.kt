...
class ProfileFragment : Fragment(R.layout.fragment_profile) {
    ...
    private companion object {
        const val API_KEY_UNLOCK_CODE = "1433223"
    }
    ...
    private fun setupMainSettings(view: View) {
        ...
        view.findViewById<View>(R.id.btn_manage_ai_rules)?.setOnClickListener {
            startActivity(Intent(requireContext(), AiRuleManageActivity::class.java))
        }
        ...
        fun refreshAiFeatureEntryVisibility(enabled: Boolean) {
            layoutAiFeatureEntry.visibility = View.GONE
            dividerAiFeatureEntry.visibility = View.GONE
        ...
        fun updateDataEntriesUi(flipEnabled: Boolean, multiCurrencyEnabled: Boolean) {
            ...
            btnFlipSensitivity.visibility = View.VISIBLE
    ...
    private fun showEditUserProfileDialog() {
        ...
        val dialog = AlertDialog.Builder(ctx)
            ...
            .setPositiveButton("保存") { _, _ ->
                ...
                val shouldOpenApiKeyDialog = rawDesc.trim() == API_KEY_UNLOCK_CODE
                if (!shouldOpenApiKeyDialog) {
                    Prefs.setUserProfileDesc(ctx, rawDesc)
                }
                rootRef?.let { refreshUserAvatarCard(it) }
                if (shouldOpenApiKeyDialog) {
                    showHiddenApiKeyDialog()
                } else {
    ...
    private fun showHiddenApiKeyDialog() {
        if (!isAdded) return
        val ctx = requireContext()
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    val result = runCatching {
                        withContext(Dispatchers.IO) {
                            AIService.fetchModelsWithDetails(Prefs.getAiUrl(ctx), key)
                        }
                    }
                    ...
                    result.onSuccess { models ->
                        ...
                        Prefs.setAiKey(ctx, key)
                        if (cleanedModels.isNotEmpty()) {
                            Prefs.setAiModelsCache(ctx, cleanedModels)
                            Utils.toast(ctx, "AI Key 已保存，模型已就绪")
                        } else {
                            Utils.toast(ctx, "AI Key 已保存，但未获取到模型列表")
                        }
                        dialog.dismiss()
                    }.onFailure { error ->
                        ...
                        Utils.toast(
                            ctx,
                            when {
                                msg.contains("timeout", ignoreCase = true) -> "连接 AI 服务超时，请稍后重试"
                                msg.contains("Failed to connect", ignoreCase = true) ||
                                    msg.contains("Unable to resolve host", ignoreCase = true) -> "无法连接 AI 服务，请检查 API 地址"
                                msg.contains("401") -> "认证失败：API Key 可能错误或已过期"
                                else -> "测试失败: ${msg.ifBlank { error.javaClass.simpleName }}"
                            }
                        )
                    }
                }
