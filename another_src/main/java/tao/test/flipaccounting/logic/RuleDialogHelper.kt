...
不要做任何多余解释，只返回关键字文本本身。例如：豆浆,拉面 微信"""
    ...
    fun showDialog(
        ...
        val themeCtx = ContextThemeWrapper(ctx, tao.test.flipaccounting.R.style.Theme_FlipAccounting)
        ...
        if (!referenceText.isNullOrEmpty()) {
            tvReferenceText.visibility = View.VISIBLE
            tvReferenceText.text = "参考文本：" + referenceText
            btnAiExtract.visibility = View.VISIBLE
        } else {
            tvReferenceText.visibility = View.GONE
            btnAiExtract.visibility = View.GONE
        }
        ...
        // Accounts
        CoroutineScope(Dispatchers.IO).launch {
            val accounts = Prefs.getAssets(ctx).map { it.name }
            withContext(Dispatchers.Main) {
        ...
        val builder = AlertDialog.Builder(themeCtx)
            .setTitle(if (rule == null) "添加专属规则" else "编辑专属规则")
            .setView(view)
            .setPositiveButton("保存") { _, _ ->
                val keyword = etKeyword.text.toString().trim()
                if (keyword.isEmpty()) {
                    Toast.makeText(ctx, "关键字不能为空", Toast.LENGTH_SHORT).show()
                ...
                // Split multiple keywords and save multiple rules?
                val keywords = keyword.split("，", ",").map { it.trim() }.filter { it.isNotEmpty() }

                for (kw in keywords) {
                    val newRule = AiRule(
                        id = if (keywords.size == 1) (rule?.id ?: java.util.UUID.randomUUID().toString()) else java.util.UUID.randomUUID().toString(),
                        keyword = kw,
