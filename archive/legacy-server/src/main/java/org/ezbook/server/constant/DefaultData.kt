...
4) 人名不能作为账户名。
5) cateName 必须严格从 Category Data（按收入/支出区分）中选择；无精确匹配则填“其它”。
...
- 必须严格从 Category Data 中选择；若无法确定，输出 其它。
...
现在请基于以下财务数据生成分析报告（只输出完整的JSON对象，不要其它内容）：
""".trimIndent()
