...
class AssetUnarchiveTool(private val db: AppDatabase) : AgentTool {
    override val id = "asset.unarchive"
    override val category = "资产"
    override val risk = RiskLevel.WRITE
    override val description = "取消收纳（取消归档）资产账户"
    override val parameterSchema = JSONObject().apply {
        put("type", "object")
        put("properties", JSONObject().apply {
            put("assetName", JSONObject().apply {
                put("type", "string")
                put("description", "要取消收纳的资产名称")
            })
        })
    ...
    override suspend fun validate(params: JSONObject, context: AgentSessionContext): AgentValidationResult {
        ...
        val matches = allAssets.filter { it.name.contains(assetName, ignoreCase = true) }
        if (matches.isEmpty()) {
    ...
    override suspend fun execute(params: JSONObject, context: AgentSessionContext): AgentToolResult {
        ...
        if (!asset.isArchived) {
            return AgentToolResult.success(userMessage = "资产「${asset.name}」未被收纳")
        }

        db.assetDao().updateArchived(asset.id, false)
        return AgentToolResult.success(
            facts = JSONObject().apply {
                put("assetId", asset.id)
                put("assetName", asset.name)
            },
            userMessage = "已取消收纳资产「${asset.name}」"
        )
    }
}
