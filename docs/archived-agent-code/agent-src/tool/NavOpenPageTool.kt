...
class NavOpenPageTool(private val context: Context) : AgentTool {
    ...
    override suspend fun validate(params: JSONObject, context: AgentSessionContext): AgentValidationResult {
        ...
        if (page.isEmpty()) {
            return AgentValidationResult.invalidParams("请指定要打开的页面", listOf("page"))
        }
        if (page !in allowedPages) {
            return AgentValidationResult.invalidParams(
                "不支持的页面: $page，可选: ${allowedPages.joinToString(", ")}",
                listOf("page")
            )
        }
    ...
    override suspend fun execute(params: JSONObject, context: AgentSessionContext): AgentToolResult {
        val page = params.optString("page", "").trim().lowercase()
        if (page !in allowedPages) {
            return AgentToolResult.failure("不支持的页面: $page")
        }

        val intent = when (page) {
            "home" -> Intent(this.context, MainActivity::class.java).apply {
                putExtra("tab", "home")
            }
            "stats" -> Intent(this.context, MainActivity::class.java).apply {
                putExtra("tab", "stats")
            }
            "assets" -> Intent(this.context, MainActivity::class.java).apply {
                putExtra("tab", "assets")
            }
            "settings" -> Intent(this.context, MainActivity::class.java).apply {
